...
void LevelEditor::createLevelForTargetWindow()
{
	...
	//first call parents method to render the basics.
	...
	//math is from whatever is remaining after the environment split, give 60% to items
	...
	//int balance = this->_level_window->getWindowWidth() - this->_X_Cor_EnvironmentPortionEnd_ItemPortionStart;
	balance = (balance / 100.00) * 60;
...
void LevelEditor::renderEnvironmentAtBottom()
{
	...
	dest.w = (this->_X_Cor_CharacterPortionEnd_DescriptionPortionStart - this->_X_Cor_ItemPortionEnd_CharacterPortionStart)* 0.9;
	...
	this->getLevelWindow()->setFontType(9);
	this->getLevelWindow()->addTextLabel("Environment, Items and Enemy", 255,0,0, this->_environmentTextDestination);
	this->getLevelWindow()->setTitleFontSize(72);
	this->getLevelWindow()->setMenuOnRenderer();
}
...
void LevelEditor::renderItemsAtBottom()
{
	...
	//this->getLevelWindow()->setFontType(9);
	//this->getLevelWindow()->addTextLabel("", 255,0,0, this->_itemsTextDestination);
	//this->getLevelWindow()->setTitleFontSize(72);
	//this->getLevelWindow()->setMenuOnRenderer();
}
...
void LevelEditor::renderCharactersAtBottom()
{
	//first render the text
	this->getLevelWindow()->setFontType(9);
	this->getLevelWindow()->addTextLabel("Player", 255,0,0, this->_characterTextDestination);
	this->getLevelWindow()->setTitleFontSize(72);
	this->getLevelWindow()->setMenuOnRenderer();
...
void LevelEditor::renderDescriptionAtBottomRIght(GameComponent* component)
{
	...
	//if it is not environment then it is parent gamecomponent so player
	for(int x=0; x<_tempText.size(); x++)
	{
		if(_tempText[x]!=nullptr)
		{
